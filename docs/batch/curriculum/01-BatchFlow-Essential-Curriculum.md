...
> 더 깊은 주제(비동기 처리, 성능 튜닝, JobOperator/스케줄링/모니터링, 대량 알림 프로젝트)는
> [00 문서의 Phase 4~6 (Step 27~50)](./00-BatchFlow-Curriculum.md)을 참조하세요.
...
| 테이블 | 용도 | 시드 |
|--------|------|------|
| member | 휴면 전환 대상 (status, last_login_at) | ~50명 (ACTIVE 30 = 정상 20 + 휴면대상 10 / DORMANT 15 / WITHDRAWN 5) |
| transaction | 일일 정산의 원천 (type, amount) | ~200건 |
