...
테스트 코드가 자산이 됐습니다. 그런데 테스트가 지키는 건 아직 "동작"뿐입니다 —
리뷰에서 매번 반복하는 그 지적들은요?

> "Controller에서 DAO 직접 부르시면 안 돼요" — 이 규칙, 신규 입사자가 어기면 누가 잡죠?

동작은 테스트가 지키는데 **구조**는 여전히 사람이 지킵니다. 그리고 사람의 규율은
마감 앞에서 집니다. 구조도 테스트로 봉인할 수 있다면? —
**심화 Step 13: ArchUnit**에서 다룹니다. (이어서 Step 14: REST Docs — "문서"까지
테스트의 보호 아래로. Step 12 View가 남았다면 그쪽 먼저!)

마지막 당부 한 가지 — **다음에 코드를 작성할 때, 테스트를 먼저(혹은 같이) 쓰세요.**
배운 모든 것은 습관이 되었을 때만 힘이 됩니다.
막히면 언제든 치트시트(`docs/test/skills/spring-test-annotations.md`)로 돌아오세요.
